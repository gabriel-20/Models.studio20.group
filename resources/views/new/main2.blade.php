...
<script>
        ...
        setInterval(function() {
            ...
            jQuery.ajax({
                ...
                success: function (html) {
                    ...
                    //console.log(html);
                    $('#totalModels').text(html.totalModels);
    ...
    function otherModelAddon(){
        ...
        jQuery.ajax({
            ...
            success: function (html) {
                ...
                var array = JSON.parse(html.result);

                var total = parseInt($('#totalModels').text());
                $('#totalModels').text(total + array.length);

                generateOtherModels(array);
    ...
    function generateOtherModels(array){
        ...
        array.forEach(function(elem) {
            ...
            //hardcode for test
            //status = 'private_chat';
            if ((status === 'private_chat') || (status === 'free_chat')) {

            var totalonline = parseInt($('#totalOnline').text());
            $('#totalOnline').text(totalonline + 1);

            if ((status === 'private_chat')){
                var private = parseInt($('#totalPrivate').text());
                $('#totalPrivate').text(private + 1);
            }

            if ((status === 'free_chat')){
                var free = parseInt($('#totalFree').text());
                $('#totalFree').text(free + 1);
            }

            var id = (elem.id);
            var studio = (elem.studio);
            var name = (elem.name);
            var surname = (elem.surname);
            var avatar = (elem.pic);

            var twitter = (elem.twitter);
            var Modelname = (elem.nickname);
            var last_change = '2018';
            var newImg = '';

            var modelnamehtml = '<h4 class="mt-4 modelImage">' + Modelname + '</h4>';
            var lastchangehtml = '<p>Updated: <strong>' + last_change + '</strong></p>';
            var relocatebtn = '';
            if (status === 'private_chat') relocatebtn = '<a href="javascript:relocate_home(\'' + Modelname + '\')" class="btn btn-sm btn-primary">Private Chat</a>';
            if (status === 'free_chat') relocatebtn = '<a href="javascript:relocate_home(\'' + Modelname + '\')" class="btn btn-sm btn-success">Free Chat</a>';
            if (status === 'offline') relocatebtn = '<a href="javascript:relocate_home(\'' + Modelname + '\')" class="btn btn-sm btn-danger">Offline</a>';
            avatar = '<img id=' + id + ' src=' + avatar + ' class="rounded-circle" alt="" style="height: 100px!important;">';

            var elemhtml = '<div class="col-sm-6 col-xl-3 ' + status + '"><div class="card"><div class="card-body"><div class="text-center"><div style="display: none;"></div>';
            elemhtml = elemhtml + newImg + avatar + modelnamehtml + lastchangehtml + relocatebtn + '</div></div></div></div>';

            var script = '<script>var id =  ' + id + ';var img = ' + avatar + ';if (img === null){ getData("' + Modelname + '", ' + id + ');}</' + 'script>';
